...
export default function ActionBar({
  ...
  // دالة ذكية للبحث عن رقم الجوال ونسخه
  const copyPhone = () => {
    if (!selected) return;
    const text = selected.data.markdown_data || "";
    // كود بحث عن أي أرقام تبدو كرقم هاتف (بدون التأثير على التواريخ)
    const phoneRegex =
  ...
  const copyName = () => {
    ...
    navigator.clipboard.writeText(selected.data.name);
      ...
      <div className={`action-bar-container ${isCollapsed ? "collapsed" : ""}`}>
        <div className="action-bar">
          ...
          >
            ...
            {editMode ? "عرض النتيجة" : "تحرير Markdown"}
          </button>

          <button
            className={`toggle-btn ${draftMode ? "draft-active" : ""}`}
            onClick={() => setDraftMode(!draftMode)}
            disabled={editMode}
          >
            <Icon name="draft" size={14} />{" "}
            {draftMode ? "إغلاق المسودة" : "مسودة"}
          </button>

          <div className="divider-v" />

          {/* الأزرار الجديدة (النسخ السريع) */}
          <button
            className="pill-btn"
            onClick={copyName}
